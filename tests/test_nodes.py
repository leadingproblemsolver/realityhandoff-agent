...
@pytest.mark.asyncio
async def test_preflight_allows_read_only_negative_control_when_action_tool_hidden(monkeypatch):
    async def fake_manifest():
        return {
            "tools": ["search", "get_entities", "list_schema_fields", "get_lineage"],
            "missing_required_read": [],
            "missing_required_action": ["update_description"],
            "read_ready": True,
            "action_ready": False,
        }

    monkeypatch.setattr(nodes.mcp_runtime, "capability_manifest", fake_manifest)
    result = await nodes.preflight_node({"task": "fix revenue definition"})
    assert result["status"] == "DATAHUB_MCP_READY"
    assert result["stage"] == "MCP_PREFLIGHT"


def test_ready_contract_fails_closed_if_mutation_tool_not_exposed():
    state = state_for_write()
    state["gate"] = GateResult(decision="READY", reason="ok").model_dump()
    state["tool_manifest"] = {"missing_required_action": ["update_description"]}
    result = nodes.contract_node(state)
    assert result["stage"] == "FAILED"
    assert result["status"] == "CONTRACT_FAILED"
    assert "update_description" in result["error"]
